...
bp_data[!, :std_pp] = standardizes(bp_data[!, :PP_shifted])
...
@time nm = trajgwas(@formula(std_pp ~ 1 + sex_genetic + std_age + std_age_sq + std_age_sex + std_bmi +
        PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10),
    @formula(std_pp ~ 1 + std_age),
    @formula(std_pp ~ 1 + sex_genetic + std_age + std_age_sq + std_age_sex + std_bmi +
        PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10),
    :eid,
    bp_data,
    nothing;
    nullfile="/gdata01/user/xuhe/SPA-GRM/real_data-2022-12-29/null_fitter/pp_nm.txt",
    solver=solver,
    runs=10)

SubjID = unique(nm.ids)
# extract the residuals for beta from fitted null model, note that the residuals must match id!!!
f1 = open("/gdata01/user/xuhe/SPA-GRM/real_data-2022-12-29/null_fitter/pp_beta_resid.txt", "w")
...
f2 = open("/gdata01/user/xuhe/SPA-GRM/real_data-2022-12-29/null_fitter/pp_tau_resid.txt", "w")
...
bp_data_un[!, :std_pp] = standardizes(bp_data_un[!, :PP_shifted])
...
@time nm_un = trajgwas(@formula(std_pp ~ 1 + sex_genetic + std_age + std_age_sq + std_age_sex + std_bmi +
        PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10),
    @formula(std_pp ~ 1 + std_age_indicator),
    @formula(std_pp ~ 1 + sex_genetic + std_age + std_age_sq + std_age_sex + std_bmi +
        PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10),
    :eid,
    bp_data_un,
    nothing;
    nullfile="/gdata01/user/xuhe/SPA-GRM/real_data-2022-12-29/null_fitter/pp_nm_un.txt",
    solver=solver,
    runs=10)

open("/gdata01/user/xuhe/SPA-GRM/real_data-2022-12-29/null_fitter/pp_nm_un.jls", "w") do io
        Serialization.serialize(io, nm_un)
end

# using Serialization
# nm_un = open(deserialize, "/gdata01/user/xuhe/SPA-GRM/real_data-2022-12-29/null_fitter/pp_nm_un.jls")
