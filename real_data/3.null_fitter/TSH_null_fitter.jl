...
TSH_data = CSV.read("/gdata01/user/xuhe/SPA-GRM/real_data-2022-12-29/final_pheno/TSH.txt", DataFrame)

TSH_data = filter(x -> x.BMI !== missing, TSH_data)

# standardize
standardizes(x) = (x .- mean(skipmissing(x))) ./ std(skipmissing(x))

TSH_data[!, :std_tsh] = standardizes(TSH_data[!, :tsh])
TSH_data[!, :std_age] = standardizes(TSH_data[!, :age])
TSH_data[!, :std_age_sq] = map(x -> x.std_age ^ 2, eachrow(TSH_data))
TSH_data[!, :std_age_sex] = TSH_data[!, :std_age] .* TSH_data[!, :sex_genetic]
TSH_data[!, :std_bmi] = standardizes(TSH_data[!, :BMI])

TSH_data[!, :age_indicator] = map(x -> round(x.age), eachrow(TSH_data))
TSH_data[!, :std_age_indicator] = standardizes(TSH_data[!, :age_indicator])

# use WiSER to fit null model, note that we call WiSER by trajgwas without given genofile.
@time nm = trajgwas(@formula(std_tsh ~ 1 + sex_genetic + std_age + std_age_sq + std_age_sex + std_bmi +
    ...
    TSH_data,
    nothing;
    nullfile="/gdata01/user/xuhe/SPA-GRM/real_data-2022-12-29/null_fitter/TSH_nm.txt",
    solver=solver,
...
f1 = open("/gdata01/user/xuhe/SPA-GRM/real_data-2022-12-29/null_fitter/TSH_beta_resid.txt", "w")
...
f2 = open("/gdata01/user/xuhe/SPA-GRM/real_data-2022-12-29/null_fitter/TSH_tau_resid.txt", "w")
...
TSH_data_un = filter(x -> x.UNRELATED_WB == true, TSH_data)

TSH_data_un[!, :std_tsh] = standardizes(TSH_data_un[!, :tsh])
TSH_data_un[!, :std_age] = standardizes(TSH_data_un[!, :age])
TSH_data_un[!, :std_age_sq] = map(x -> x.std_age ^ 2, eachrow(TSH_data_un))
TSH_data_un[!, :std_age_sex] = TSH_data_un[!, :std_age] .* TSH_data_un[!, :sex_genetic]
TSH_data_un[!, :std_bmi] = standardizes(TSH_data_un[!, :BMI])

TSH_data_un[!, :age_indicator] = map(x -> round(x.age), eachrow(TSH_data_un))
TSH_data_un[!, :std_age_indicator] = standardizes(TSH_data_un[!, :age_indicator])

@time nm_un = trajgwas(@formula(std_tsh ~ 1 + sex_genetic + std_age + std_age_sq + std_age_sex + std_bmi + 
        PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10),
    @formula(std_tsh ~ 1 + std_age_indicator),
    @formula(std_tsh ~ 1 + sex_genetic + std_age + std_age_sq + std_age_sex + std_bmi),
    :eid,
    TSH_data_un,
    nothing;
    nullfile="/gdata01/user/xuhe/SPA-GRM/real_data-2022-12-29/null_fitter/TSH_nm_un.txt",
    solver=solver,
    solver_config = solver_config,
    runs=30,
    init = x -> WiSER.init_ls!(x; gniters=0))

open("/gdata01/user/xuhe/SPA-GRM/real_data-2022-12-29/null_fitter/TSH_nm_un.jls", "w") do io
        Serialization.serialize(io, nm_un)
end

# using Serialization
# nm_un = open(deserialize, "/gdata01/user/xuhe/SPA-GRM/real_data-2022-12-29/null_fitter/TSH_nm_un.jls")
