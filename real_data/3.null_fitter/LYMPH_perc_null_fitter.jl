...
LYMPH_data = CSV.read("/gdata01/user/xuhe/SPA-GRM/real_data-2022-12-29/final_pheno/LYMPH_perc.txt", DataFrame)
...
LYMPH_data[!, :std_lymph_perc] = standardizes(LYMPH_data[!, :lymph_perc])
...
@time nm = trajgwas(@formula(std_lymph_perc ~ 1 + sex_genetic + std_age + std_age_sq + std_age_sex + std_bmi +
        PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10),
    @formula(std_lymph_perc ~ 1 + std_age_indicator),
    @formula(std_lymph_perc ~ 1 + sex_genetic + std_age + std_age_sq + std_age_sex + std_bmi),
    :eid,
    LYMPH_data,
    nothing;
    nullfile="/gdata01/user/xuhe/SPA-GRM/real_data-2022-12-29/null_fitter/LYMPH_perc_nm.txt",
    solver=solver,
...
f1 = open("/gdata01/user/xuhe/SPA-GRM/real_data-2022-12-29/null_fitter/LYMPH_perc_beta_resid.txt", "w")
...
f2 = open("/gdata01/user/xuhe/SPA-GRM/real_data-2022-12-29/null_fitter/LYMPH_perc_tau_resid.txt", "w")
...
LYMPH_data_un[!, :std_lymph_perc] = standardizes(LYMPH_data_un[!, :lymph_perc])
...
@time nm_un = trajgwas(@formula(std_lymph_perc ~ 1 + sex_genetic + std_age + std_age_sq + std_age_sex + std_bmi + 
        PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10),
    @formula(std_lymph_perc ~ 1 + std_age_indicator),
    @formula(std_lymph_perc ~ 1 + sex_genetic + std_age + std_age_sq + std_age_sex + std_bmi),
    :eid,
    LYMPH_data_un,
    nothing;
    nullfile="/gdata01/user/xuhe/SPA-GRM/real_data-2022-12-29/null_fitter/LYMPH_perc_nm_un.txt",
    solver=solver,
    solver_config = solver_config,
    runs=10)

open("/gdata01/user/xuhe/SPA-GRM/real_data-2022-12-29/null_fitter/LYMPH_perc_nm_un.jls", "w") do io
        Serialization.serialize(io, nm_un)
end

# using Serialization
# nm_un = open(deserialize, "/gdata01/user/xuhe/SPA-GRM/real_data-2022-12-29/null_fitter/LYMPH_perc_nm_un.jls")
