...
pr_data = CSV.read("/gdata01/user/xuhe/SPA-GRM/real_data-2022-12-29/final_pheno/pulse_rate.txt", DataFrame)

pr_data = filter(x -> x.BMI !== missing, pr_data)

# standardize
standardizes(x) = (x .- mean(skipmissing(x))) ./ std(skipmissing(x))

pr_data[!, :std_pr] = standardizes(pr_data[!, :pr])
pr_data[!, :std_age] = standardizes(pr_data[!, :age])
pr_data[!, :std_age_sq] = map(x -> x.std_age ^ 2, eachrow(pr_data))
pr_data[!, :std_age_sex] = pr_data[!, :std_age] .* pr_data[!, :sex_genetic]
pr_data[!, :std_bmi] = standardizes(pr_data[!, :BMI])

pr_data[!, :age_indicator] = map(x -> round(x.age), eachrow(pr_data))
pr_data[!, :std_age_indicator] = standardizes(pr_data[!, :age_indicator])

# use WiSER to fit null model, note that we call WiSER by trajgwas without given genofile.
@time nm = trajgwas(@formula(std_pr ~ 1 + sex_genetic + std_age + std_age_sq + std_age_sex + std_bmi +
        PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10),
    @formula(std_pr ~ 1 + std_age_indicator),
    @formula(std_pr ~ 1 + sex_genetic + std_age + std_age_sq + std_age_sex + std_bmi),
    :eid,
    pr_data,
    nothing;
    nullfile="/gdata01/user/xuhe/SPA-GRM/real_data-2022-12-29/null_fitter/pulse_rate_nm.txt",
    solver=solver,
...
f1 = open("/gdata01/user/xuhe/SPA-GRM/real_data-2022-12-29/null_fitter/pulse_rate_beta_resid.txt", "w")
...
f2 = open("/gdata01/user/xuhe/SPA-GRM/real_data-2022-12-29/null_fitter/pulse_rate_tau_resid.txt", "w")
...
pr_data_un = filter(x -> x.UNRELATED_WB == true, pr_data)

pr_data_un[!, :std_pr] = standardizes(pr_data_un[!, :pr])
pr_data_un[!, :std_age] = standardizes(pr_data_un[!, :age])
pr_data_un[!, :std_age_sq] = map(x -> x.std_age ^ 2, eachrow(pr_data_un))
pr_data_un[!, :std_age_sex] = pr_data_un[!, :std_age] .* pr_data_un[!, :sex_genetic]
pr_data_un[!, :std_bmi] = standardizes(pr_data_un[!, :BMI])

pr_data_un[!, :age_indicator] = map(x -> round(x.age), eachrow(pr_data_un))
pr_data_un[!, :std_age_indicator] = standardizes(pr_data_un[!, :age_indicator])

@time nm_un = trajgwas(@formula(std_pr ~ 1 + sex_genetic + std_age + std_age_sq + std_age_sex + std_bmi +
        PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10),
    @formula(std_pr ~ 1 + std_age_indicator),
    @formula(std_pr ~ 1 + sex_genetic + std_age + std_age_sq + std_age_sex + std_bmi),
    :eid,
    pr_data_un,
    nothing;
    nullfile="/gdata01/user/xuhe/SPA-GRM/real_data-2022-12-29/null_fitter/pulse_rate_nm_un.txt",
    solver=solver,
    solver_config = solver_config,
    runs=10)

open("/gdata01/user/xuhe/SPA-GRM/real_data-2022-12-29/null_fitter/pulse_rate_nm_un.jls", "w") do io
