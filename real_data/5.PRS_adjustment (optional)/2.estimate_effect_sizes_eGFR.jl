...
# sbatch -J PRS --mem=15G -t 3-0:0 -o log/test_for_PRS_longitudinal_eGFR.log --wrap='julia /gdata01/user/xuhe/family_relatedness/real_data-2023-08-29/4.PRS_new/select_markers_longitudinal_new_step2_eGFR.jl'
...
pheno_data[!, :std_egfr] = standardizes(pheno_data[!, :egfr])
...
for chr in 1:22
    ...
    @time trajgwas(@formula(std_egfr ~ 1 + sex_genetic + std_age + std_age_sq + std_age_sex + std_bmi +
        PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10),
    # @formula(std_egfr ~ 1 + std_age_indicator),
    @formula(std_egfr ~ 1), # more efficient
