...
# compared with longitudinal_pheno_simu_function-2023-02-16XH.R, I plan to use log of inverse gamma to generate wi.
                      ...
                      bvectoWS = NULL)                # random effect passes to WS variance, match the family structure.
{
  ...
  gamma = MASS::mvrnorm(n = N, mu = rep(0, q), Sigma = diag(q))
  omega = log(invgamma::rinvgamma(n = N, shape = 5, rate = 10))
  gamma_omega = cbind(gamma, omega) %*% chol(cov)
